...
with open('cipher.csv') as text:
    cipher = csv.reader(text, delimiter = ',')
    chars = np.array([int(bytePair) for line in cipher for bytePair in line], dtype='uint8')

ASCII = {char: index for index, char in enumerate(ascii_lowercase, 97)}
np.repeat([1, 2], 3)